...
export default function SchedulePage() {
    ...
    useEffect(() => {
        if (id) {
            api.get<MortgageResponse>(`/mortgage-profiles/${id}`).then(setData).finally(() => {
                setLoading(false);
    ...
    if (!data?.mortgagePaymentSchedule) {
        return (
            <div className="p-4">
                <h1 className="text-2xl font-bold mb-4">График платежей</h1>
                <p className="text-[#a1a1aa]">Нет данных о графике</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-4 pb-20 opacity-0 animate-fade-in" style={{ animationFillMode: 'forwards' }}>
            <button
                onClick={() => router.push('/')}
                className="mb-4 flex items-center gap-2 text-[#a1a1aa] hover:text-white transition-colors"
            >
                    ...
                    <div key={year} className="mb-8">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-xs">
                                <thead className="bg-[#1c1c1f] border border-[#3f3f46]">
